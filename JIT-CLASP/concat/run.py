...
from sklearn.metrics import roc_auc_score, auc, accuracy_score, precision_recall_fscore_support, \
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            (inputs_ids, attn_masks, msg_input_ids,msg_input_mask,manual_features, labels) = [x.to(args.device) for x in batch]
            model.train()
            loss, logits, _ = model(inputs_ids, attn_masks, msg_input_ids,msg_input_mask,manual_features, labels)
            if args.n_gpu > 1:
                loss = loss.mean()
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    for batch in eval_dataloader:
        (inputs_ids,attn_masks, msg_input_ids,msg_input_mask,manual_features, labels ) = [x.to(args.device) for x in batch]
        with torch.no_grad():
            loss, logit, _ = model(inputs_ids,attn_masks, msg_input_ids,msg_input_mask,manual_features, labels )
...
def test(args, model, tokenizer, best_threshold=0.5):
    ...
    for batch in eval_dataloader:
        (inputs_ids,attn_masks, msg_input_ids,msg_input_mask,manual_features, labels ) = [x.to(args.device) for x in batch]
        with torch.no_grad():
            loss, logit, attn_weights = model(inputs_ids, attn_masks, msg_input_ids,msg_input_mask,manual_features, labels, output_attentions=True)
            last_layer_attn_weights = attn_weights
            eval_loss += loss.mean().item()
            logits.append(logit.cpu().numpy())
            y_trues.append(labels.cpu().numpy())
            probs.extend(logit.cpu().numpy().tolist())
    ...
    auc_roc = roc_auc_score(y_trues, probs)
    auc_pr = auc_pc(y_trues, probs)
    auc_pr2 = average_precision_score(y_trues, probs)
    acc = accuracy_score(y_trues, y_preds)
    precision, recall, f1, _ = precision_recall_fscore_support(
        y_trues, y_preds, average='binary')
    mcc = matthews_corrcoef(y_trues, y_preds)

    df = pd.DataFrame({'label': y_trues, 'pred': probs})
    save_path = "{}/checkpoint-best-f1/predictions_dp.csv".format(args.output_dir)
    ...
    result = {
        ...
        "eval_f1": float(f1),
        "eval_mcc": float(mcc),
        "eval_AUC-ROC": float(auc_roc),
        "eval_AUC-PR": float(auc_pr),
        "eval_AUC-PR2": float(auc_pr2),
        "eval_acc": float(acc),
        "eval_threshold": best_threshold,
...
def parse_args():
    ...
    parser.add_argument('--head_dropout_prob', type=float, default=0.1,
                        help="Number of labels")
    parser.add_argument('--no_abstraction', type=bool, default=True,
                        help="Number of labels")

    parser.add_argument('--alpha', type=float, default=0.5,
                        help="The loss weight of comparison loss")
    parser.add_argument('--temp', type=float, default=0.1,
                        help="The temperture of comparison loss")
