...
class InputFeatures(object):
    ...
    def __init__(self, commit_id, input_ids, input_mask, msg_input_ids,msg_input_mask,input_tokens, label, manual_features):
        self.commit_id = commit_id
        self.input_ids = input_ids
        self.input_mask = input_mask
        self.msg_input_ids = msg_input_ids
        self.msg_input_mask = msg_input_mask
...
def convert_examples_to_features(item, cls_token='[CLS]', sep_token='[SEP]', sequence_a_segment_id=0,
    ...
    input_tokens = ['[ADD]'] + added_tokens + ['[DEL]'] + removed_tokens
    ...
    msg_tokens = msg_tokens[:min(args.max_msg_length, len(msg_tokens))-2]
    msg_tokens = [tokenizer.cls_token] + msg_tokens + [tokenizer.sep_token]
    msg_input_ids = tokenizer.convert_tokens_to_ids(msg_tokens)
    msg_input_mask = [1 if mask_padding_with_zero else 0] * len(msg_input_ids)

    # Zero-pad up to the sequence length.
    code_padding_length = 512 - len(input_ids)
    input_ids = input_ids + ([pad_token] * code_padding_length)
    input_mask = input_mask + ([0 if mask_padding_with_zero else 1] * code_padding_length)

    msg_padding_length = args.max_msg_length - len(msg_input_ids)
    msg_input_ids = msg_input_ids + ([pad_token] * msg_padding_length)
    msg_input_mask = msg_input_mask + ([0 if mask_padding_with_zero else 1] * msg_padding_length)

    assert len(input_ids) == 512
    assert len(input_mask) == 512
    # assert len(msg_input_ids) == 512
    # assert len(msg_input_mask) == 512

    return InputFeatures(commit_id=commit_id,
                         input_ids=input_ids,
                         input_mask=input_mask,
                         msg_input_ids=msg_input_ids,
                         msg_input_mask=msg_input_mask,
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, item):
        return (torch.tensor(self.examples[item].input_ids),
                ...
                torch.tensor(self.examples[item].msg_input_ids),
                torch.tensor(self.examples[item].msg_input_mask),
